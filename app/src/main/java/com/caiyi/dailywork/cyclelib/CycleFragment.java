package com.caiyi.dailywork.cyclelib;
...
public class CycleFragment extends Fragment implements OnPageChangeListener {
    ...
    public void setData(List<SimpleDraweeView> views, List<ADInfo> list, ImageCycleViewListener listener, int showPosition) {
        ...
        if (isCycle) {
            ...
            indicators = new ImageView[ivSize - 3];
            indicatorLayout.removeAllViews();
        }
    ...
    /**
     * 设置轮播暂停时间，即过多少秒切换到下一张视图.默认5000ms
     *
     * @param time 毫秒为单位
     */
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i]
                    .setBackgroundResource(R.drawable.gjj_circle_dot_white_2);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition]
                    .setBackgroundResource(R.drawable.gjj_circle_dot_red);
    }
